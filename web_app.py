...
WILLINGNESS_BASENAME = "Willingness_Admin_View"
LEGACY_WILLINGNESS_FILE = "Willingness.xlsx"
FINAL_ALLOTMENT_FILE = "Final_Allocation.xlsx"
ALLOTMENT_CONTROL_FILE = "allotment_control.json"
...
def resolve_willingness_file():
    candidates = [
        f"{WILLINGNESS_BASENAME}.xlsx",
        f"{WILLINGNESS_BASENAME}.xls",
        f"{WILLINGNESS_BASENAME}.csv",
        LEGACY_WILLINGNESS_FILE,
    ]
    for f in candidates:
        if os.path.exists(f):
            return f
    return f"{WILLINGNESS_BASENAME}.xlsx"


def save_willingness(df):
    target = resolve_willingness_file()
...
def load_allotment_publish_state():
    if os.path.exists(ALLOTMENT_CONTROL_FILE):
        try:
            with open(ALLOTMENT_CONTROL_FILE, "r", encoding="utf-8") as f:
                payload = json.load(f)
            return bool(payload.get("faculty_allotment_visible", False))
        except Exception:
            return False
    return False


def save_allotment_publish_state(is_visible):
    payload = {"faculty_allotment_visible": bool(is_visible)}
    with open(ALLOTMENT_CONTROL_FILE, "w", encoding="utf-8") as f:
...
def load_willingness():
    willingness_file = resolve_willingness_file()
    if os.path.exists(willingness_file):
        if str(willingness_file).lower().endswith('.csv'):
            df = pd.read_csv(willingness_file)
        else:
...
if "faculty_allotment_visible" not in st.session_state:
    st.session_state.faculty_allotment_visible = load_allotment_publish_state()
...
if panel_mode == "Admin View":
    ...
    if not st.session_state.admin_authenticated:
        ...
        st.markdown("#### Allotment Publish Control")
        current_publish = load_allotment_publish_state()
        st.info(f"Faculty Allotment Visibility: {'Enabled' if current_publish else 'Disabled'}")
        c_pub1, c_pub2 = st.columns(2)
        with c_pub1:
            if st.button("Enable Allotment to Faculty", key="enable_allotment", use_container_width=True):
                save_allotment_publish_state(True)
                st.session_state.faculty_allotment_visible = True
                st.success("Allotment is now visible to faculty.")
                st.rerun()
        with c_pub2:
            if st.button("Disable Allotment to Faculty", key="disable_allotment", use_container_width=True):
                save_allotment_publish_state(False)
                st.session_state.faculty_allotment_visible = False
                st.success("Allotment is now hidden from faculty.")
                st.rerun()
...
if user_panel_mode == "Allotment":
    if not load_allotment_publish_state():
        st.info("Allotment is currently hidden by admin. Once all willingness entries are completed and admin allows, faculty allotment will be shown here.")
        st.markdown("---")
        st.markdown("Curated by Dr. N. Sathiya Narayanan | School of Mechanical Engineering")
        st.stop()
